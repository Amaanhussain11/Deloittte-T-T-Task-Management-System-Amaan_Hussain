...
function Avatar({ name }) {
  ...
  return (
    <div className="w-6 h-6 rounded-full bg-accent/20 border border-accent/30 flex items-center justify-center text-[10px] font-mono text-accent shrink-0">
      {initials}
    </div>
  )
}
  ...
  return (
    <div className="flex items-center gap-4 px-5 py-3.5 border-b border-border/60 hover:bg-surface-2/40 transition-colors group animate-fade-in">
      ...
      <div className="w-36 shrink-0 hidden md:flex items-center gap-2">
        {task.assignedTo ? (
          <>
            ...
            <span className="text-xs text-slate-400 truncate">{task.assignedTo.name}</span>
          </>
        ) : (
      ...
      <div className="w-32 shrink-0 hidden lg:block">
        <span className="text-xs text-slate-600 truncate block">{task.createdBy?.name}</span>
      </div>
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        {canEdit && (
          <button onClick={() => onEdit(task)}
            className="text-xs text-slate-400 hover:text-slate-200 px-2 py-1 rounded hover:bg-surface-3 transition-colors">
            Edit
          </button>
        )}
        {isAdmin && (
    ...
    <div className="flex items-center justify-between px-5 py-3 border-t border-border">
      ...
      <div className="flex items-center gap-1">
        <button
          ...
          className="text-xs px-2 py-1 rounded text-slate-400 hover:text-slate-200 hover:bg-surface-3 disabled:opacity-30 disabled:cursor-not-allowed transition-colors"
        >
          ← Prev
        </button>
            ...
            <button
              ...
              className={`text-xs w-7 h-7 rounded font-mono transition-colors ${
                p === page
                  ...
                  : 'text-slate-500 hover:text-slate-200 hover:bg-surface-3'
              }`}
            >
              {p + 1}
            </button>
      ...
      <div className="p-6 max-w-7xl mx-auto">
        ...
        {search && (
          <div className="mb-3 flex items-center gap-2">
            ...
            <span className="text-xs bg-accent/15 text-accent border border-accent/30 rounded-md px-2 py-0.5 font-mono">
              "{search}"
            </span>
        ...
        <div className="card overflow-hidden">
          ...
          <div className="flex items-center gap-4 px-5 py-2.5 border-b border-border bg-surface-2/50">
            <div className="w-28 shrink-0 text-[10px] font-mono text-slate-600 uppercase tracking-widest">Status</div>
            <div className="flex-1 text-[10px] font-mono text-slate-600 uppercase tracking-widest">Title</div>
            <div className="w-36 shrink-0 hidden md:block text-[10px] font-mono text-slate-600 uppercase tracking-widest">Assignee</div>
            <div className="w-32 shrink-0 hidden lg:block text-[10px] font-mono text-slate-600 uppercase tracking-widest">Created by</div>
            <div className="w-24 shrink-0 hidden xl:block text-[10px] font-mono text-slate-600 uppercase tracking-widest">Date</div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-16 gap-2">
              ...
              {hasFilters
                ? <button className="text-accent text-sm hover:underline" onClick={clearFilters}>Clear filters</button>
                : <button className="text-accent text-sm hover:underline" onClick={() => setShowCreate(true)}>Create your first task →</button>
              }
            </div>
          ) : (
            tasks.map(task => (
